...
You can deploy this project to Coolify even before buying your own domain, as long as your Coolify instance already provides autogenerated subdomains or has a wildcard domain configured.

Recommended setup:

1. Create a new `Application` from the GitHub repository.
2. Prefer a `Node.js / Nixpacks` style deployment if you want the simplest setup.
...
5. Set the port to:
   `3000`
6. Add the required environment variables from `.env.example`.
7. Set `NEXT_PUBLIC_SITE_URL` and `NEXTAUTH_URL` to the Coolify-generated URL/subdomain.
8. Configure the health check path as:
   `/api/health`

If you prefer Docker-based deployment in Coolify, this repository also includes a Dockerfile that now runs `prisma db push` before starting the Next.js server.

Do not run `npm run seed` automatically on every deployment. Use it only once if you want demo content and the seeded admin account.
